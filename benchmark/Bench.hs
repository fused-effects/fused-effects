{-# LANGUAGE DeriveFunctor, FlexibleInstances, LambdaCase, MultiParamTypeClasses, RankNTypes, TypeApplications, UndecidableInstances #-}
module Main
( main
) where
...
instance (Algebra sig m, Effect c sig) => Algebra sig (Cod m) where
