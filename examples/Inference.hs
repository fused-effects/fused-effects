...
askEnv :: (Has (Reader env) sig m, Effect sig, Constrain sig Identity) => HasEnv env m env
askEnv = ask
...
instance (Algebra sig m, Effect sig, Constrain sig Identity) => Algebra sig (HasEnv env m) where
