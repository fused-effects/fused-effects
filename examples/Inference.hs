...
askEnv :: (Has (Reader env) sig m, Effect sig) => HasEnv env m env
askEnv = ask
...
instance (Algebra sig m, Effect sig) => Algebra sig (HasEnv env m) where
  alg = HasEnv . handleIdentity runHasEnv
