{-# LANGUAGE ConstraintKinds, DeriveFunctor, FlexibleInstances, MultiParamTypeClasses, RankNTypes, TypeOperators, UndecidableInstances #-}
...
instance (Algebra sig m, Effect c sig, c (CutC Identity)) => Algebra (Cut :+: NonDet :+: sig) (CutC m) where
  ...
  alg (R (R other))          = CutC $ \ cons nil fail -> handle (pure ()) dst other >>= runIdentity . runCut (coerce cons) (coerce nil) (coerce fail) where
