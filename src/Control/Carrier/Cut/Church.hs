...
instance (Algebra sig m, CanThread sig (CutC PureC)) => Algebra (Cut :+: NonDet :+: sig) (CutC m) where
  ...
  alg (R (R other))          = CutC $ \ cons nil fail -> handle (pure ()) dst other >>= run . runCut (coerce cons) (coerce nil) (coerce fail) where
