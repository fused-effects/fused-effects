...
instance (Algebra sig m, Effect sig, Constrain sig (Either String)) => Fail.MonadFail (FailC m) where
  fail = send . Fail
  {-# INLINE fail #-}

instance (Algebra sig m, Effect sig, Constrain sig (Either String)) => Algebra (Fail :+: sig) (FailC m) where
