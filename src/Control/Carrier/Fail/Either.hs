...
instance (Algebra sig m, CanHandle c sig (Either String)) => Fail.MonadFail (FailC m) where
  fail = send . Fail
  {-# INLINE fail #-}

instance (Algebra sig m, CanHandle c sig (Either String)) => Algebra (Fail :+: sig) (FailC m) where
