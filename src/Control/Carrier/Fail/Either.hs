...
instance (Algebra sig m, Effect Identity sig, Effect (Either String) sig) => Fail.MonadFail (FailC m) where
  fail = send . Fail
  {-# INLINE fail #-}

instance (Algebra sig m, Effect Identity sig, Effect (Either String) sig) => Algebra (Fail :+: sig) (FailC m) where
