...
instance (Algebra sig m, Effect c sig, c (Either String)) => Fail.MonadFail (FailC m) where
  fail = send . Fail
  {-# INLINE fail #-}

instance (Algebra sig m, Effect c sig, c (Either String)) => Algebra (Fail :+: sig) (FailC m) where
